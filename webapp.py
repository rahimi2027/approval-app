# ============================================================
# 🔄 ACOOLE PORTAL — PROFESSIONAL VERSION v2.5 (ALL BUGS FIXED)
# ============================================================
# ✅ st.set_page_config restored as FIRST Streamlit call
# ✅ Stray top-level "with tab_audit:" block REMOVED
# ✅ PERMISSION_DEFAULTS.get() — no KeyError on custom roles
# ✅ Safe pdf.output() for fpdf2 AND classic PyFPDF
# ✅ pdf.dashed_line() replaced (removed in fpdf2 >= 2.7.14)
# ✅ PDF generation gated behind button press (no rerun spam)
# ✅ init_user_db no longer wipes existing users
# ✅ Unique widget keys everywhere (no DuplicateWidgetID)
# ✅ "Send to Director" -> "Submit Request"
# ============================================================
...
# ============================================================
st.set_page_config(page_title="Acoole Portal", page_icon="⚡", layout="wide")
...
def upload_to_google_drive(local_file_path, display_filename):
    try:
        if not SERVICE_ACCOUNT_INFO:
            st.warning("⚠️ No Google credentials — file kept on local storage only.")
            return None
...
# ✅ Fallback used for any custom role not present in PERMISSION_DEFAULTS
FALLBACK_PERMS = {"can_view_all_dept": False, "can_generate_pdf": False,
                  "can_download_data": False, "can_approve_requests": False}
...
def display_audit_log_panel():
    ...
    with c1:
        filter_user = st.multiselect("👤 Filter by User", sorted(set([l["User_Name"] for l in logs])), key="audit_f_user")
    with c2:
        dept_list = sorted(set([l.get("Department", "") for l in logs if l.get("Department") != "-"]))
        filter_dept = st.multiselect("🏢 Filter by Department", dept_list, key="audit_f_dept")
    with c3:
        filter_action = st.multiselect("🔧 Filter by Action", sorted(set([l["Action"] for l in logs])), key="audit_f_action")
    with c4:
        req_list = sorted(set([str(l["Request_ID"]) for l in logs if str(l["Request_ID"]) != "-"]))
        filter_req = st.multiselect("🆔 Filter by Request ID", req_list, key="audit_f_req")
    ...
    st.download_button("📥 Download Full Audit Log (CSV)", df_export.to_csv(index=False).encode("utf-8"),
                       "Acoole_Audit_Log.csv", type="primary", key="dl_audit_csv")
...
def init_user_db():
    ...
    # ✅ Only seed defaults when the file is genuinely EMPTY (content-based, not size-based)
    try:
    ...
    try:
        ...
        for _, r in df.iterrows():
            ...
            if not uname:
                continue
            users[uname] = {
                "full_name": str(r.get("full_name", uname)).strip(),
...
def _pdf_to_bytes(pdf):
    """✅ Works with BOTH fpdf2 (returns bytearray) and classic PyFPDF (returns str)."""
    try:
...
def display_pdf_button(req, can_generate=False, key_suffix=""):
    """✅ PDF is only built when the button is pressed (not on every rerun)."""
    req_id = req["id"]
    ...
    if can_generate and PDF_AVAILABLE:
        if st.button(f"📄 Generate PDF for ID #{req_id}", type="primary", key=unique_key):
            ...
            if ok:
                st.session_state[f"pdf_cache_{unique_key}"] = (pdf_bytes, name)
            else:
                st.error(f"❌ {name}")
        cached = st.session_state.get(f"pdf_cache_{unique_key}")
        if cached:
            st.success("✅ Generated! Ready to download ⬇")
            st.download_button(f"📥 Download: {cached[1]}", data=cached[0], file_name=cached[1],
...
def settings_management_panel():
    ...
    st.info("🛡️ Super Admin Only — Add, Edit, Delete Categories, Departments & Roles.")
    st.divider()
    ...
    with cats_tab:
        ...
        st.info("These appear in the request form dropdown.")
        st.divider()
        ...
        for i, cat in enumerate(current_cats):
            ...
            with c2:
                if st.button("✏️ Edit", key=f"edit_cat_{i}"):
                    st.session_state[f"editing_cat_{i}"] = True
            with c3:
                if len(current_cats) > 1 and st.button("🗑️ Delete", key=f"del_cat_{i}"):
    ...
    with dept_tab:
        ...
        st.info("Create, rename or remove departments.")
        st.divider()
        ...
        for i, dept_name in enumerate(current_depts):
            ...
            with c2:
                if st.button("✏️ Edit", key=f"edit_dept_{i}"):
                    st.session_state[f"editing_dept_{i}"] = True
            with c3:
                if len(current_depts) > 1 and st.button("🗑️ Delete", key=f"del_dept_{i}"):
    ...
    with roles_tab:
        ...
        st.info("⚠️ 'Super Admin' cannot be deleted.")
        st.divider()
        ...
        with st.form("add_role_form", clear_on_submit=True):
            ...
            if st.form_submit_button("✅ Add Role"):
                if new_role.strip() and new_role.strip() not in current_roles:
                    ...
                    default_perms = PERMISSION_DEFAULTS.get(new_role.strip(), FALLBACK_PERMS)
        ...
        for i, role in enumerate(current_roles):
            ...
            with c3:
                if role != "Super Admin" and len(current_roles) > 1 and st.button("🗑️ Delete", key=f"del_role_{i}"):
...
def user_management_panel():
    ...
    with tab1:
        ...
        with st.form("create_user_form", border=True, clear_on_submit=True):
            ...
            defaults = PERMISSION_DEFAULTS.get(new_role, FALLBACK_PERMS)   # ✅ no KeyError
            col1, col2 = st.columns(2)
    ...
    with tab3:
        ...
        current_uname = st.session_state.get("user_info", {}).get("username", "")
        deletable = [u for u in USERS.keys() if u != current_uname]
        if deletable:
            del_user_sel = st.selectbox("Select User to DELETE", deletable, key="del_user_selector")
            if del_user_sel:
                del_name = USERS[del_user_sel].get("full_name", del_user_sel)
                if st.button(f"🗑️ DELETE: {del_name} ({del_user_sel})", type="secondary", key="confirm_del_user"):
            ...
            st.info("📋 No deletable users (you cannot delete your own account).")
...
def create_pdf_from_request(req):
    try:
        ...
        # ✅ dashed_line() was REMOVED in fpdf2 >= 2.7.14 — draw it manually instead
        ...
        if not pending:
            ...
            for req in reversed(pending):
                with st.expander(f"🟡 ID #{req.get('id')} | {req.get('emp_name')} | £{float(req.get('amount',0)):.2f}"):
                    ...
                    st.divider()
                    display_pdf_button(req, can_generate=True, key_suffix="pay_p")
        ...
        if not approved:
            ...
            for req in reversed(approved):
                ...
                with st.expander(title):
                    ...
                    display_attachments(req)
                    st.divider()
                    display_pdf_button(req, can_generate=True, key_suffix="pay_a")
        ...
        if not rejected:
            ...
            for req in reversed(rejected):
                with st.expander(f"🔴 ID #{req.get('id')} | {req.get('emp_name')} | £{float(req.get('amount',0)):.2f}"):
                    ...
                    display_attachments(req)
                    st.divider()
                    display_pdf_button(req, can_generate=True, key_suffix="pay_r")

# ─── MANAGER / STAFF / TEAM MEMBER PORTAL ───
    ...
    if st.session_state.get("editing_request_id"):
        ...
        if rec:
            ...
            show_old_new_comparison(rec.get("old_data", "{}"), rec)
            ...
            with st.form("edit_form"):
                ...
                with c1:
                    ...
                    rt = st.selectbox("🔄 Transaction Type", ["Addition", "Deduction"],
                        index=["Addition", "Deduction"].index(rec.get("type", "Addition")) if rec.get("type") in ["Addition", "Deduction"] else 0)
                    ...
                    amt = st.number_input("💷 Amount (£)", min_value=0.01, step=10.0, value=float(rec.get("amount", 0.01)) or 0.01)
                with c2:
                    from datetime import datetime as dt
                    try:
                        d = dt.strptime(str(rec.get("date", ""))[:10], "%Y-%m-%d")
                    except Exception:
                        d = dt.today()
            ...
            st.warning("⚠️ Request not found.")
            if st.button("⬅️ Back", key="back_from_edit"):
                st.session_state.editing_request_id = None
                st.rerun()
        ...
        with st.form("new_req", clear_on_submit=True):
            ...
            # ✅ "Send to Director" option REMOVED — now a neutral Submit button
                    ...
                    try:
                        ...
                        if files:
                            for i, f in enumerate(files, 1):
                                ...
                                att_list.append(fn)
                                # Drive upload is best-effort — never blocks the submission
                                try:
                                    upload_to_google_drive(file_path, fn)
                                except Exception as drive_err:
                                    st.warning(f"⚠️ Drive upload skipped: {str(drive_err)[:150]}")
                        ...
                        st.error(f"❌ Submission failed: {type(submit_err).__name__}: {submit_err}")
                else:
        ...
        if not my_reqs:
            ...
            for req in reversed(my_reqs):
                ...
                with st.expander(label):
                    ...
                    if status == "approved":
                        display_pdf_button(req, can_generate=False, key_suffix="mgr_a")
    ...
    with tab_approved:
        ...
        if not approved:
            ...
            for req in reversed(approved):
                ...
                dec_by = req.get('decision_by', 'Director')
                dec_date = req.get('decision_date', '')
                label = (f"🟢 ID #{req_id} | {req.get('emp_name')} | APPROVED | £{float(req.get('amount',0)):.2f} | 📅 {format_date(dec_date)} "
                         f"| ✅ Approved by: {dec_by} | ⏰ {dec_date or '—'}")
                with st.expander(label):
                    st.write(f"👤 Employee: {req.get('emp_name')} | 🏢 Department: {req.get('dept','')}")
                    st.write(f"💷 Amount: £{float(req.get('amount',0)):.2f}")
                    st.write(f"🎯 Approved By: {dec_by}")
                    st.write(f"📅 Approval Date: {dec_date or '—'}")
                    st.info(f"💬 Comments: {req.get('director_comments', 'None')}")
                    display_attachments(req)
                    st.divider()
                    display_pdf_button(req, can_generate=True, key_suffix="dir_a")
...
# ─── SUPER ADMIN PORTAL ───
            ...
            for req in reversed(pending):
                ...
                with st.expander(title):
                    ...
                    if req.get("director_comments"):
                        st.info(f"💬 Director Comments: {req.get('director_comments')}")
                    st.divider()
                    display_pdf_button(req, can_generate=True, key_suffix="sa_p")
    ...
    with tab_approved:
        ...
        if not approved:
            ...
            for req in reversed(approved):
                ...
                dec_by = req.get('decision_by', 'Director')
                dec_date = req.get('decision_date', '')
                display_date = format_date(dec_date)
                label = (
                ...
                with st.expander(label):
                    ...
                    display_attachments(req)
                    st.divider()
                    display_pdf_button(req, can_generate=True, key_suffix="sa_a")
        ...
        if not rejected:
            ...
            for req in reversed(rejected):
                ...
                with st.expander(title):
                    ...
                    display_attachments(req)
                    st.divider()
                    display_pdf_button(req, can_generate=True, key_suffix="sa_r")
    ...
    with tab_manage:
        ...
        with tab_audit:
            # ✅ Clear-History button lives HERE (inside the real tab_audit)
            _ui = st.session_state.get("user_info", {})
            if _ui.get("role", "") == "Super Admin":
                st.divider()
                col_btn, col_info = st.columns([1, 3])
                with col_btn:
                    if st.button("🗑️ Clear History Log", type="secondary", key="clear_audit_btn"):
                        archive_path, count = archive_audit_log()
                        clear_audit_log_file()
                        archived_name = os.path.basename(archive_path) if archive_path else "no previous log found"
                        st.success(f"✅ Log cleared! Archived {count} entries → `{archived_name}`")
                        st.rerun()
                with col_info:
